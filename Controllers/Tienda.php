			...
			if($_POST){
				...
				if(is_numeric($idproducto) and ($option == 1 or $option == 2)){
					...
					$arrResponse = array("status" => true, 
											...
											"cantCarrito" => $cantCarrito,
											"htmlCarrito" => $htmlCarrito,
											"subTotal" => SMONEY.formatMoney($subtotal),/*formatmoney*/
											"total" => SMONEY.formatMoney($subtotal + COSTOENVIO)/*formatmoney*/
										);
				}else{
					$arrResponse = array("status" => false, "msg" => 'Dato incorrecto.');
				}
			...
			if($_POST){
				...
				if(is_numeric($idproducto) and $cantidad > 0){
					...
					$arrResponse = array("status" => true, 
										...
										"totalProducto" => SMONEY.($totalProducto),
										"subTotal" => SMONEY.($subtotal),
										"total" => SMONEY.($subtotal + COSTOENVIO)
									);
